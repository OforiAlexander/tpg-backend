...
class CommentsController {
  /**
     * Get comments for a ticket - FIXED VERSION
  ...
  async getComments(req, res) {
    try {
      ...
      // Build main query for fetching comments
      let query = TicketComment.query()
        .where('ticket_id', ticketId)
        .withGraphFetched('[user, attachments]');

      // Build separate count query (without joins)
      let countQuery = TicketComment.query()
        .where('ticket_id', ticketId);
  ...
  async addComment(req, res) {
    try {
      ...
      try {
        ...
        if (ticket.status === 'open' && req.user.hasPermission('tickets.view.all') && !finalIsInternal) {
          await ticket.$query().patch({
            status: 'in-progress',
