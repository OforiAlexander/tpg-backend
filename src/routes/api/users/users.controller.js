...
class UsersController {
  ...
  async getUsers(req, res) {
    try {
      ...
      // Build base query with filters (without sorting)
      ...
      // Create count query BEFORE applying sorting
      ...
      if (validSortFields.includes(sortBy)) {
        dataQuery = dataQuery.orderBy(sortBy, sortOrder === 'asc' ? 'asc' : 'desc');
      }

      // Apply pagination
      const offset = (parseInt(page) - 1) * parseInt(limit);
      dataQuery = dataQuery.offset(offset).limit(parseInt(limit));
    ...
    try {
      ...
      if (!authService.isValidTPGEmail(email)) {
        return res.status(400).json({
          ...
          message: 'Only @upsamail.edu.gh email addresses are allowed'
        });
      }
  ...
  async getUsersStatistics(req, res) {
    try {
      ...
      const [
        ...
        // Active users count
        User.query().where('status', 'active').count('id as count').first(),

        // Pending users count  
        User.query().where('status', 'pending').count('id as count').first(),

        // Suspended users count
        User.query().where('status', 'suspended').count('id as count').first(),

        // Locked users count
        User.query().where('status', 'locked').count('id as count').first(),

        // Admin users count (admin + super_admin)
        User.query().whereIn('role', ['admin', 'super_admin']).count('id as count').first(),

        // Regular users count
        User.query().where('role', 'user').count('id as count').first(),

        // Email verified users count - Fixed field name
        User.query().whereNotNull('email_verified_at').count('id as count').first(),
        ...
        // Recently active users (last 30 days based on last_login)
        User.query()
          .whereNotNull('last_login')
          .where('last_login', '>=', new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString())
          .count('id as count')
          .first()
      ]);
      ...
      const statistics = {
        ...
        pending: parseInt(pendingUsers?.count || 0),
        suspended: parseInt(suspendedUsers?.count || 0),
        locked: parseInt(lockedUsers?.count || 0),
        admins: parseInt(adminUsers?.count || 0),
        regular_users: parseInt(regularUsers?.count || 0),
        email_verified: parseInt(emailVerifiedUsers?.count || 0),
        recent_registrations: parseInt(recentRegistrations?.count || 0),
        last_30_days_active: parseInt(recentlyActiveUsers?.count || 0)
      };

      // Calculate additional metrics
      const totalNonSuspended = statistics.total - statistics.suspended;
      const verificationRate = statistics.total > 0
        ? Number(((statistics.email_verified / statistics.total) * 100).toFixed(1))
        : 0;

      const activeRate = totalNonSuspended > 0
        ? Number(((statistics.active / totalNonSuspended) * 100).toFixed(1))
        : 0;

      // Add calculated metrics
      statistics.verification_rate = verificationRate;
      statistics.active_rate = activeRate;
      statistics.growth_rate = statistics.total > 0
        ? Number(((statistics.recent_registrations / statistics.total) * 100).toFixed(1))
        : 0;
      ...
      logger.error('Get users statistics error:', {
        error: error.message,
        stack: error.stack,
        user_id: req.user?.id,
        method: req.method,
